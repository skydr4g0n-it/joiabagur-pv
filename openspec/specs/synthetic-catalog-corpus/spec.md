# synthetic-catalog-corpus Specification

## Purpose
Offline CLI in `jbg_ai.data` that generates a versioned JSONL corpus of synthetic jewelry products (SKU reserved by code, name/description/price reasoned by OpenAI, provenance stamped in code, quality tiers by name stem, new design collections) and inserts those collections and products into local PostgreSQL without touching the C06a real anchor or family tables. The HTTP service does not import this package; `/health` boots without LLM keys.
