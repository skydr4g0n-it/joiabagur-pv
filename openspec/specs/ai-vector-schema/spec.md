# ai-vector-schema Specification

## Purpose
Vector persistence layer of the `jbg-ai` service: ownership of PostgreSQL schema `ai` and the prohibition on writing to `public`, provisioning of the `vector` extension and of a dedicated least-privilege role ahead of migrations, a reversible migration cycle whose version table lives inside the owned schema, the shape and integrity invariants of the vector index tables (including `text_provenance` and `sync_checkpoint`), the indexing guarantees that fail silently when wrong — cosine operator class aligned with the query operator, GIN-backed array and full-text filters, Spanish text search configuration anchored to the schema — and a bounded, lazily created connection pool.
## Requirements
