# family-review Specification

## Purpose
Standing review of the families the catalogue already holds, as distinct from proposing new ones: a deterministic audit that recomputes over persisted families, from the vectors already on the index and without calling any model, the members a product of another family sits closer to than their own worst sibling, and the unassigned products that beat a family's worst-sibling similarity by a configured margin — nominating by that relative margin rather than by neighbourhood purity, which selects for the near-duplicate families the synthetic corpus was built with. The audit is a read and writes nothing. Covers the human verdict persisted per `(product, family)` pair with its reviewer, its instant and the margin observed at the time, honoured by omitting that pair from later audits and never reopened silently; the visible gap between a recorded judgement and the membership that would honour it, and the enactment that closes it through the same path that stamps the catalog watermark; correction of a member's variant label after the fact; review time recorded per judgement rather than held in a screen, with an absence reported instead of a zero and the two populations counted apart; administrator-only access to every operation; and the rule that a list which could not be computed is never presented as a list that came back empty.

Two operations reach the screen from outside the audit's reach. **Creating a family by hand**, with its members and their variant labels in one declaration, exists because the audit nominates an unassigned product by its margin *relative to a target family*: a product whose piece type has no family at all cannot be nominated, since there is nothing to compute a margin against, and no tuning of a threshold reaches it. **Keyboard operation of the queue** — approving, rejecting and advancing without a pointing device, and inert while focus sits inside a text-editing field — is shared with the profile review queue so a reviewer moving between the two screens meets one set of keys rather than two. The proof that matters for it is a queue worked with them by a person, not a handler attached; that proof was obtained, and it carries a verdict worth recording: the shortcuts work and do not measurably shorten a review, because the cost of this task is reading the source text and deciding, not moving a hand.
