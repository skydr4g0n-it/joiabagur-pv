# family-suggestion Specification

## Purpose
Assisted grouping of catalogue products into families of variants, and the batch approval of those groups. Covers the deterministic grouping engine — normalized name root within one piece type, size removed from any position, material as a fusion axis rather than a stripped token, and guards that report what they refuse instead of dropping it — the embedding veto that is relative to the other proposals and marks rather than removes, variant labels kept verbatim on the size axis and canonical on the material one with member order from an internal size rank, the separation between proposing, which writes nothing, and applying, which is the only write path and records the administrator who approved, and the restriction of both operations to administrators.
