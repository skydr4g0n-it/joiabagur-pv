# sales-assist-card Specification

## Purpose
Sale card of the frontend: a lazily loaded route of its own anchored to one piece and one point of sale, reachable from the assisted search result row, from the product chosen on the manual sale page and from the piece resolved by scanning a code, with the role-resolved point-of-sale selector as the fallback when none travels in navigation state and no request issued before one is chosen, exactly one sale assistance request per visit with no automatic retry and a further one only on an explicit act — the retry of a failed request, naming a different point of sale, or the customer's question — that question bounded at the maximum the backend declares and sent in the request body so that it never reaches the address of the page nor the browser's history, the closed warning vocabulary translated in a module of its own with a neutral label for a code this screen does not know, the missing size label rendered as an attribute of the piece beside its SKU rather than as an alert that would crowd out the two stock warnings, a family of two or more members requiring the operator to name the variant with nothing preselected and the member chosen handed to the manual sale page through navigation state, the six states of the argument told apart as five messages that never merge the two differing in provenance, citations carrying their scope with the establishment ones marked to be confirmed in store and none displayed when the argument is not, substitutes requested from the anchored member's stock and never on a degraded card with their four outcomes distinguishable and a short page declared, and every failure of the two backend routes rendered as a true sentence rather than an application error, with neither the customer's question nor the resolved argument ever written to the browser console.
