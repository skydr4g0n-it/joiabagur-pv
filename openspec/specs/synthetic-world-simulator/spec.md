# synthetic-world-simulator Specification

## Purpose
Offline CLI in `jbg_ai.data.world` (`python -m jbg_ai.data world simulate|ingest`) that, from a committed YAML census of twelve POS, simulates biased inventory and a coherent Poisson sales history against the local catalog (natural keys, non-negative stock, ephemeral co-occurrence) and inserts it into Docker PostgreSQL without touching catalog tables, schema `ai`, OpenAPI, or the `jbg-ai` HTTP runtime.
