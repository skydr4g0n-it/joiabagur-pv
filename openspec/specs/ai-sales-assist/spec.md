# ai-sales-assist Specification

## Purpose
The two routes of the backend's sale card, a write-free `POST /api/ai/products/{productId}/sales-assist` and a `GET /api/ai/products/{productId}/substitutes`: the point-of-sale scope and the anchored product both validated against the transactional catalog before a single call is spent on the AI service, authoritative hydration of the group the index returns so that name, price, quantity, photo and collection come from the catalog of that shop and a member it does not carry disappears, the two stock warnings computed here and the variants warning narrowed to the members that survive hydration, the price and stock placeholders resolved against the anchored product with the argument — and never the response — withheld when any placeholder remains, six explicit states of the argument the screen can tell apart, substitutes requested once with the largest window the frozen contract allows and reduced to what the shop can sell today with four distinguishable empty outcomes, degradation that serves the card from the backend's own family records when the AI service cannot answer, a switch per point of sale, a request-rate policy of its own for the generative route, and a logging policy in which neither the customer's question nor the resolved argument ever appears.
