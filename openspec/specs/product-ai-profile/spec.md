# product-ai-profile Specification

## Purpose
Reviewable AI profile of a catalog product: one profile per product enforced by the database, the confidence and provenance of every proposed field, and the hybrid per-field review policy that follows from them — a sensitive attribute a model inferred needs a person, the same attribute from a deterministic rule does not, and commercial tags auto-approve above a configured threshold. Review status and review origin are independent, so an indexing feed can select by status while human-review metrics select by origin without either lying. Covers batch enrichment restricted to administrators, its idempotency over the enrichment inputs, its behaviour when it loses a race or when the extractor has no implementation yet, the storage that later review metrics will need, and the schema properties whose incorrect value raises no error at all.
## Requirements
