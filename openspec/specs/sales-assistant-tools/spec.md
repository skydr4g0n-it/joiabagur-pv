# sales-assistant-tools Specification

## Purpose
The catalogue of read-only tools the sale assistant calls, and nothing that calls them. The registry holds exactly six tools — `buscar_catalogo`, `buscar_sustitutos`, `listar_familia`, `consultar_conocimiento`, `consultar_disponibilidad` and `pedir_aclaracion` — and the set of names is a declared constant rather than whatever the construction happened to assemble, so that a seventh tool is a deliberate act with a test behind it and the two tools withdrawn before this capability existed — a point-of-sale profile whose service was cancelled, and complementary products whose signals measured empty — cannot return by accident. Every tool publishes a name, a Spanish description written for a reader who sees no code and a typed parameter schema, and validates the arguments it receives against that schema before any port is touched, bounded parameters carrying their bounds explicitly and enumerated ones their closed set; the description is the interface the model reads to choose, so it is a prompt and it is versioned with the code that serves it. That no tool writes is verified structurally, by introspecting the constructed registry on three axes — the set of registered names, the methods exposed by every port a tool captures, and the HTTP verbs any registered client can issue — rather than by a boolean on the descriptor, which is set by whoever registers it and is precisely who could be wrong: the published limitation that no agent writes has to be demonstrated or it must not be published. A failure never escapes as an exception, which would kill the consuming loop instead of costing it one turn; it comes back as an observation marked failed carrying a cause drawn from a closed vocabulary of codes, the Spanish left to whoever presents it, and an unknown piece reference is one of those observations and not an error. Availability is reported as a qualitative label from a closed vocabulary containing no digit and no quantity of stock — the .NET side remains the authority on stock, and a bucket emitted verbatim would be the beginning of one — declaring the age of the projection reading that produced it, taken from the checkpoint the drain already records, and degrading rather than failing when that reading is stale; the absence of a reading scope or of a projection row is a value of its own and never an absence of stock, because collapsing the two would pivot to substitutes over a piece the shop can actually sell. Clarification selects a missing axis from the closed set the assistance layer already publishes and resolves the Spanish question in code from the existing closed catalogue of templates, never written by a model, since that field is typed as prose and has no net under it. Tools address pieces by SKU and never by internal product identifier, and observations carry only what a consumer needs to decide the next step and no raw retrieval score — everything returned is re-sent on every subsequent turn of the consuming loop, and scores are not comparable across tools — so an ordering that matters travels as a position. No tool calls a chat completion provider, which is what makes this layer measurable at no cost and the ablation against it clean, and the embedding calls some tools legitimately make are counted in a counter of their own rather than in the provider-call figure the assistance layer already publishes. The registry is wired to no route of its own: its consumer is the agent loop, which serves `POST /v1/assist/agent` from the `sales-assistant-agent` capability — that route, and the movement of the published contract it brought, belong to that capability — and sale assistance is unaffected in any of its three modes. Tests run offline, driving injected fakes and reaching no provider, no network and no real database.
