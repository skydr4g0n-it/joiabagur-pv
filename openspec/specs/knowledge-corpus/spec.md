# knowledge-corpus Specification

## Purpose
The second index of the system. `ai.product_document` answers questions about products and cannot answer questions about the jewellery or the service, because those answers live in no product. Commercial knowledge is authored as Markdown documents versioned in the repository, one file per document, general and never scoped to a product: no section names a stock keeping unit, an individual product or a price, and none states a count or a proportion of the assortment, so that adding or withdrawing a product invalidates no document and no sentence expires in silence while its citation keeps resolving. There are no sales scripts — the corpus keeps **facts to cite** and the prompt keeps **instructions to obey**, because an imperative fragment retrieved into a prompt is indistinguishable from an instruction. Every section declares its `claim_scope`, `general` for what is verifiable outside the jewellery and `establecimiento` for a commitment of the establishment, and that declaration travels with the chunk the section produces so a consumer can present the two differently. Coverage of the material sheets is derived from the enrichment closed vocabulary rather than chosen by hand. Chunking is one second-level section per chunk, without overlap, with the document title and the section title inside the indexed content so both take part in the embedding and in the full-text vector the schema generates; an oversized section fails ingestion instead of being split, because an automatic split produces a fragment with no heading of its own. Identity is deterministic over the slugs and never over the position of the section, and the citation identifier resolves to a file and a heading of the versioned corpus. Indexing is idempotent, skips the embedding when neither the content hash nor the embedding version changed, removes what a run no longer produces, and creates no table, column or index. Knowledge embeddings carry a preprocessing version of their own, separate from the key that versions the product text renderer. Search is a callable of the service and not an HTTP route: a vector branch and a lexical branch fused by rank through the existing fusion function, with an abstention threshold configured separately from the product one, below which no chunk is returned at all. The corpus is accompanied by a machine-readable record of how it was produced, and its retrieval quality is measured over a versioned fixture without reading or writing the evaluation tables.
