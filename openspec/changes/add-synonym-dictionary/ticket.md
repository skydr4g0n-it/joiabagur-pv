...
- **Rendimiento / free-tier:** `expand_query` es aritmética de diccionarios sobre una consulta de ≤ 500 caracteres; presupuesto objetivo **< 1 ms**, despreciable frente a los 170–1707 ms del embed. El diccionario se carga **una vez** con `lru_cache`, como `load_vocabularies()`. **Cero llamadas nuevas al proveedor y cero consultas SQL nuevas**: es el motivo por el que no toca la rama vectorial.
