...
A term recorded as a vocabulary gap MUST name the change that will close it when one is open, and MUST otherwise record why it stays open. It MUST be removed from the exclusions once the base vocabulary defines it as a canonical: a term the base now knows is no longer a gap, and leaving it listed as one would make the file contradict the vocabulary it overlays. The error the loader raises for an unknown anchor MUST point at the base vocabulary rather than at a single named change, so that the guidance does not outlive the change it names. An overlay entry MUST NOT be added for a surface form that reaches no document, unless the entry is a regional variant declared as such.
...
- **AND** the error raised for the unknown anchor points at the base vocabulary rather than at a single named change
- **AND** any term still recorded as a vocabulary gap either names the change that will close it or records why it stays open
