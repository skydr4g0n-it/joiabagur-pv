...
**Vocabulario cerrado de materiales** declarado en Python con **test de fijación**, duplicando deliberadamente el de [`materials-vocabulary.ts`](../../../frontend/src/lib/materials-vocabulary.ts) que C16 replicó en el frontend. Cruzar la frontera para compartir un fichero de `frontend/` sería peor que la duplicación; el test hace ruidosa cualquier divergencia futura y la deuda queda anotada.
...
- [ ] **Test de fijación del vocabulario de materiales** (D12), y deuda de duplicación con `materials-vocabulary.ts` anotada en `design.md`
...
| 2 | `k` del veto relativo (`mediana − k·MAD`) y número de vecinos | **`k = 2` sobre los 5 vecinos más próximos, en `pydantic-settings`**, no en el código | Calibrado contra los 6 solapamientos medidos (1,7 % de 358), que es muestra pequeña. En configuración porque C24 lo revisará: un umbral incrustado no se puede barrer |
...
| 4 | ¿Reutilizar `materials-vocabulary.ts` de C16 o declarar vocabulario propio en Python? | **Vocabulario propio en Python con test de fijación**, y la duplicación anotada como deuda | Cruzar la frontera para que `ai-service` lea un fichero de `frontend/` sería peor que la duplicación. El test de fijación hace ruidosa cualquier divergencia futura |
