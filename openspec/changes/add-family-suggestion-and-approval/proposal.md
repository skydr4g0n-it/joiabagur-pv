...
- **BREAKING** — **`POST /v1/families/suggest`**: novena ruta de la superficie `/v1`, hoy congelada en ocho. `ai-service/openapi.json` se regenera y `test_openapi_snapshot_is_stable` se actualiza en este mismo change. El movimiento de la frontera es deliberado.
