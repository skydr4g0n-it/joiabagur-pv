...
**Reason**: The lexical branch now exists. `mode=hybrid` fuses the vector and lexical branches, `mode=lexical` runs the lexical branch alone without calling the embedding provider, and `mode=vector` runs the vector branch alone. The placeholder note `vector_only_until_c21` no longer describes the system and is removed from `debug.notes`.

**Migration**: Callers that relied on `debug.notes` containing `vector_only_until_c21` to detect a vector-only response MUST read `match_reasons` on each result instead, which now reports the branches that actually produced it. `.NET` sends `mode=hybrid` and needs no change; the frozen request and response schemas are unchanged.
