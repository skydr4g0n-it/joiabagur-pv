...
- [x] 6.8 Verificar `dotnet build` y `dotnet test` sin regresión. **Build: 0 errores.** Suite: 628 tests con 46 fallos, frente a **585 con 52 fallos en HEAD sin estos cambios** (medido con la rama limpia). Los 43 tests nuevos pasan todos; los fallos son preexistentes y flaky —el recuento varía entre ejecuciones del mismo código—, concentrados en 17 clases ajenas a este change, incluidas unitarias que no tocan nada suyo. **No es un problema que este change introduzca ni deba resolver, pero conviene abrir uno que lo aborde**
