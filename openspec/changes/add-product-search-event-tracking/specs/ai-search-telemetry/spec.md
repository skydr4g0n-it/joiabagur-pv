...
The system SHALL record searches served by the existing lexical searcher — the degraded path used when the AI service is unavailable — with the same completeness as assisted searches, and MUST mark their origin so the two paths can be separated in analysis.

The retrieval duration MUST measure obtaining the candidate list regardless of its source, so the two origins remain comparable to each other.

The origin MUST be persisted as an enumeration with explicit stable values, and its mapping MUST be documented, because the table is queried by hand.
...
#### Scenario: The two origins are separable in analysis
- **WHEN** events of both origins exist
- **THEN** grouping by origin yields the assisted and the degraded populations separately
- **AND** the retrieval duration of both is expressed in the same unit and measures the same phase
