...
- **Estado actual**: contrato congelado (C02) — **10** endpoints `/v1` con modelos Pydantic completos, JWT interno HS256 y `openapi.json` versionado (ocho en C02, más `POST /v1/families/suggest` en C18a y `POST /v1/families/audit` en C18b). Las respuestas son stubs deterministas bajo `STUB_MODE`; con `STUB_MODE=false` son reales enriquecimiento (C09), índice de catálogo (C13), recuperación de productos (C14), las dos de familias (C18a, C18b), evals (C24), sustitutos (C26) y **venta asistida (C30a)**. Queda **una** ruta en 501, `POST /v1/inventory/propose` (C35), y no por falta de turno sino por una rama anulada el 2026-08-31
