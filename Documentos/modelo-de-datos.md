...
El feed HTTP de indexación (`GET /api/ai/index-feed/catalog`) **lee** este catálogo (y el perfil, la familia y el inventario) por la API; **no** añade entidad ni columna. Autenticación: header `X-Index-Feed-Key`. C13 tira (`pull`); no hay push ni tabla outbox.
