...
| **C32** | `add-sales-assistant-agent-loop` | Python | C30b, C31 | 🔴 | — · **hereda de C31** el techo observable por `usage.calls` (C31 lo dejó en **3**: 1 enrutador + 2 argumentario) y el patrón de *fail-open* como rama con test. Su tool `pedir_aclaracion` **no es** la `clarification_question` de C31, que se resuelve en código desde catálogo cerrado |
