...
- **Contrato**: 10 endpoints `/v1` congelados en `ai-service/openapi.json`, con test de snapshot. Fueron ocho hasta C18a, que añadió `POST /v1/families/suggest`, y nueve hasta C18b, que añadió `POST /v1/families/audit` — cada uno regenerando el snapshot en su propio change. La frontera se ha movido **tres veces** desde C02, y las tres a propósito: la tercera es **C30a**, que a diferencia de las dos anteriores **no añadió ninguna ruta** — movió cinco esquemas de assist y dejó las diez rutas intactas, porque el esquema congelado no podía servir a su consumidor (`family_id` obligatorio contra un catálogo donde el 58 % de los productos no tiene familia) y en ese momento la ruta tenía **cero consumidores**. Con `STUB_MODE=true` (default local) todas las rutas `/v1` sirven fixtures C02. Con `STUB_MODE=false`, enrich (C09), index (C13), `POST /v1/retrieval/products` (C14), `POST /v1/families/suggest` (C18a), `POST /v1/families/audit` (C18b), `POST /v1/evals/runs` (C24), `POST /v1/retrieval/substitutes` (C26) y `POST /v1/assist/sale` (C30a) son reales. Queda **una** en 501, y no por falta de turno: `POST /v1/inventory/propose` pertenece a una rama **anulada el 2026-08-31** y declarada como limitación, no como trabajo pendiente. C30a cerró por tanto el **último 501 cerrable** del contrato. El de enriquecimiento se renegoció en C08 para que cada valor propuesto declare su **procedencia** (`rule` \| `inferred`) además de su confianza: sin ese dato la revisión híbrida por campo no es implementable
