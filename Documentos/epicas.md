...
> **Actualizado el 2026-09-21 — C32b implementado, y la pasada refuta el número central del diseño.** **El sobrecoste de la autonomía no es ×19: es ×7,6** (0,02144 USD/petición contra los 0,00282 del pipeline determinista), medido en la pasada `293fe5c6e470` de **204 peticiones y dos brazos**. Y **la descomposición «×5 estructural de tokens × ×4 reversible de modelo» no se sostiene**: el brazo barato baja a ×1,4 —mejor de lo que el ×4 predecía— **a cambio de no servir**, porque topa un presupuesto en **54 de 82 peticiones (66 %)** contra **1 de 82** del caro. No es que elija peor: **no sabe parar**. Q-7 queda respondida en no, con el mismo desenlace que C31 midió para el clasificador y por un síntoma distinto. **Tres de los seis presupuestos se fijan por medición y dejan de ser marcadores**: tokens 120.000 → **40.000** (p95 18.781), contexto 40.000 → **30.000** caracteres (p95 4.709) y reloj 20 s → **15,0 s** (p95 9,0 s), que **tensa el peor caso declarado** en vez de confirmarlo; el techo de herramientas sube de 6 a **8** tras medir p95 5 con un 3,9 % de peticiones truncadas. **La curva de crecimiento cierra O-5:** de 1.974 a 3.470 tokens entre la vuelta 1 y la 5, casi lineal — **no hace falta compactar**, y la decisión se toma con el dato delante en vez de por precaución. **Las dos preguntas que C32a dejó abiertas quedan respondidas:** ninguna herramienta muerta, **cero nombres de herramienta inventados** —que era «el dato más informativo que la pasada podía producir» y sale vacío— y cero llamadas consecutivas idénticas; y la etiqueta cualitativa **basta para gobernar el pivote**, con 83 % donde debe y **0 % de sobre-pivote** en las tres etiquetas donde sería caro, sin rastro del colapso que C32a prohíbe. **Lo que la implementación refuta de sus propios artefactos:** el ticket **no lista `assist/tools.py`** y hubo que tocarlo, porque la observación de C32a excluye por regla `product_id`, la puntuación y el título del documento y el contrato congelado los exige — se resuelve con un registro de evidencia que es la segunda banda lateral del registro, inerte al invariante de solo-lectura **estructuralmente** y no por exención; y **un fallo de proveedor reportaba `sin_mas_herramientas` con `partial: false`**, defecto que sólo apareció cuando un límite de tasa dejó 46 respuestas indistinguibles de una completa, y que añade `fallo_proveedor` al vocabulario cerrado **hasta el contrato publicado**. Cierre: **1.557 passed / 0 failed** comparados **por nombres** contra 1.469 —**0 nombres desaparecidos**, 88 añadidos, tres tests preexistentes modificados y ninguno retirado ni renombrado—, `openspec validate --all --strict` en **59 · 0**, y `openapi.json` movido de `43f70fda…` a **`7c6a038e…`** con **0 hojas retiradas y 0 cambiadas** verificadas contra la línea base de C32a. **El golden set no se ha tocado**, con dos tests que lo comprueban en las dos direcciones. Queda **abierto y medido**: `dangling_citation` retira el **13,1 %** de los argumentarios contra el 2,2 % de la ruta determinista. Informe: [c32b-implementation-measurements.md](Proyecto%20Final%20AIEng/informes/c32b-implementation-measurements.md).
