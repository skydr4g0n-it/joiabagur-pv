...
**Conclusión adoptada: la raíz del nombre agrupa, el embedding veta.** Es la inversión del enunciado del §7.5, y ese residuo irreducible es lo que justifica —con un número, no con una afirmación— que exista revisión humana.

> **Corrección del 2026-08-31, al implementar.** El 1,7 % se midió sobre familias formadas **sólo por sufijo de talla** —24 familias reales— y **no describe** el algoritmo que este change entrega. Con L2 más fusión hay 68 familias reales, internamente más dispersas porque agrupan `pequeño` con `pequeño oro`: más riqueza, más marcas. Además, el veto se implementó primero como una prueba *dentro* del grupo (`mediana − k·MAD` contra el centroide) cuando la medición que lo justificaba era *entre* grupos; aquélla disparaba al 16,9 %, marcando al miembro menos típico de cada clúster. **La cifra honesta del algoritmo entregado es 15 de 486 miembros (3,1 %) en 5 familias**, con margen 0,05.
...
| **D10** | ~~Veto con `k = 2` sobre 5 vecinos~~ → **revisada el 2026-08-31 al implementar: veto por margen de 0,05, entre grupos**, en configuración | El veto se marca cuando un producto de **otra familia propuesta** está más cerca del miembro que su propio peor hermano, por más del margen. La versión inicial (`mediana − k·MAD` contra el centroide) era una prueba *dentro* del grupo y disparaba al 16,9 %: marcaba al miembro menos típico de cada clúster, algo que todo clúster tiene. Curva medida: 0,02→33 marcados en 18 familias · **0,05→15 en 5** · 0,08→9 en 2. Sigue en configuración porque C24 lo barrerá con el golden set |
