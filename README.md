...
- **Servicio de IA (`jbg-ai`):** pytest con el `TestClient` de FastAPI (`uv run pytest`); cubre autenticación de servicio, conformidad de los contratos, respuestas stub, extracción de catálogo con LLM falso (`tests/enrichment/`), retriever vectorial con fakes (`tests/retrieval/`), búsqueda y direccionamiento sobre el corpus de conocimiento (`tests/knowledge/`), la venta asistida completa —estructura, citas, redacción del argumentario con sus tres comprobaciones deterministas, y desde C31 el enrutador de intención con sus dos rechazos, la repregunta determinista y el *fail-open*— (`tests/assist/`), el conjunto de enrutado de 119 casos y su criterio de veto (`tests/evals/`) y estabilidad del snapshot OpenAPI. Los tests no llaman a proveedores LLM, APIs de embeddings ni RDS: el cliente generativo se conduce con un doble que revienta si alguien lo llama de verdad.
